...
ui <- fluidPage(
  title = "truffle",
  titlePanel(title = span(img(src = "logo.png", height = 90), "truffle: simple data simulation")),
  sidebarLayout(
    sidebarPanel(
      textInput("project_identifier", "Project identifier", value = "student 1"),
      ...
      numericInput("outcomes", "Number of scales", value = 3, min = 1, step = 1),
      numericInput("items", "Items per scale", value = 10, min = 1, step = 1),
      numericInput("response_options", "Number of response options (1-to-N)", value = 7, min = 2, step = 1),
      numericInput("alpha", "Approximate Cronbach's alpha", value = 0.70, min = 0.05, max = 0.999, step = 0.01),
      numericInput("r", "Approximate r among scales", value = 0.5, min = -0.99, max = 0.99, step = 0.01),
      numericInput("d", "Approximate Cohen's d between groups (experiments only)", value = 0.5, step = 0.01),
...
server <- function(input, output, session) {
  ...
  output$download_zip <- downloadHandler(
    ...
    content = function(file) {
      ...
      # write CSVs
      readr::write_csv(vals$specs,      file.path(tmpdir, "simulation_specifications.csv"))
      ...
      # zip
      zip::zipr(zipfile = file, files = list.files(tmpdir, full.names = TRUE), include_directories = FALSE)
    }
  )
}
