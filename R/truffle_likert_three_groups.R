...
truffle_likert <- function(
  ...
  if (!study_design %in% c("factorial_between2", "factorial_between3", "crosssectional")){
    stop("study_design must be one of c('factorial_between2','factorial_between3', 'crosssectional').")
  }
  ...
  # -------------------------------
  # Case 1: Three-group factorial
  # -------------------------------
  if (study_design == "factorial_between3") {
    ...
    means_above <- paste0(factors, " ~ ", d_vec, "*1", collapse = "\n")
    means_control  <- paste0(factors, " ~ 0*1", collapse = "\n")
    means_below  <- paste0(factors, " ~ 0*1", collapse = "\n")
    
    mod_above  <- paste(mod, means_treat,  sep = "\n")
    mod_control <- paste(mod, means_ctrl, sep = "\n")
    mod_below <- paste(mod, means_ctrl, sep = "\n")
  
    # simulate continuous
    dat_above_cont  <- lavaan::simulateData(mod_above,  sample.nobs = n_per_condition)
    dat_control_cont <- lavaan::simulateData(mod_control, sample.nobs = n_per_condition)
    dat_below_cont <- lavaan::simulateData(mod_below, sample.nobs = n_per_condition)
    dat_above_cont$condition  <- "above"
    dat_control_cont$condition <- "control"
    dat_below_cont$condition <- "below"
    
    # item columns
    item_cols_above  <- grep(pref_regex, names(dat_above_cont),  value = TRUE)
    item_cols_control <- grep(pref_regex, names(dat_control_cont), value = TRUE)
    item_cols_below <- grep(pref_regex, names(dat_below_cont), value = TRUE)
    
    if (!identical(sort(item_cols_above), sort(item_cols_control)) |
        !identical(sort(item_cols_above), sort(item_cols_below))) {
      stop("Item columns differ across conditions; check prefixes or model.")
    }

    # blocks in the canonical factor/prefix order
    blocks <- .split_cols_by_prefix(item_cols_control, prefixes)
    
    # discretize by blocks with per-factor levels
    dat_above_lik  <- .discretize_by_blocks(dat_above_cont,  blocks, n_levels_vec)
    dat_control_lik <- .discretize_by_blocks(dat_control_cont, blocks, n_levels_vec)
    dat_below_lik <- .discretize_by_blocks(dat_below_cont, blocks, n_levels_vec)
    
    dat_above_lik$condition  <- condition_names[1]
    dat_control_lik$condition <- condition_names[2]
    dat_below_lik$condition <- condition_names[3]
    
    dat_lik <- dat_above_lik |>
      dplyr::bind_rows(dat_control_lik) |>
      dplyr::bind_rows(dat_below_lik) |>
  ...
  # -------------------------------
  # Case 3: Single-group cross-sectional
  # -------------------------------
