...
.continuous_to_likert_by_condition <- function(
  .data,
  n_levels = 7,
  labels = NULL,
  ordered = TRUE,
  ...
  endp_list <- lapply(Ks, function(K) {
    latent2likert::discretize_density(
  ...
  if (!is.null(labels)) {
    if (is.list(labels)) {
      ...
      stop(
        "labels must be NULL, a list (length = ncol(.data)), or a character vector of length n_levels."
      )
    }
  }
  ...
  for (j in seq_len(p)) {
    ...
    if (method == "per_column") {
      ...
      if (!is.finite(sdj) || sdj <= 0) {
        stop("Column ", j, " has non-positive SD.")
      }
      z <- (xj - mu) / sdj
    } else {
      # fixed reference
      if (!is.finite(sd_ref) || sd_ref <= 0) {
        stop("sd_ref must be > 0.")
      }
      z <- (xj - mu_ref) / sd_ref
    }

    endp <- endp_list[[as.character(n_levels[j])]]
    labs_j <- if (length(lab_list[[j]]) == 0) {
      as.character(seq_len(n_levels[j]))
    } else {
      lab_list[[j]]
    }
