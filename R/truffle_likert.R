...
truffle_likert <- function(
  ...
  if (is.null(factors)) {
    factors <- paste0("X", seq_len(K), "_latent")
  }
  if (is.null(prefixes)) {
    prefixes <- paste0("X", seq_len(K), "_item")
  }

  if (!study_design %in% c("factorial_between2", "crosssectional")) {
    stop(
      "study_design must be one of c('factorial_between2','crosssectional')."
    )
  }
  ...
  if (study_design == "factorial_between2") {
    # d handling
    if (
      !is.numeric(approx_d_between_groups) ||
        any(!is.finite(approx_d_between_groups))
    ) {
      stop(
        "`approx_d_between_groups` must be numeric and finite (scalar or length-K)."
      )
    }
    if (length(approx_d_between_groups) == 1L) {
      ...
      if (!is.null(names(d_vec))) {
        if (!all(factors %in% names(d_vec))) {
          stop(
            "Named `approx_d_between_groups` must include all factor names: ",
        ...
        stop(
          "If not scalar and unnamed, `approx_d_between_groups` must have length K = ",
          K,
          "."
        )
      }
    ...
    # simulate continuous
    dat_ctrl_cont <- lavaan::simulateData(
      mod_ctrl,
      sample.nobs = n_per_condition
    )
    dat_treat_cont <- lavaan::simulateData(
      mod_treat,
      sample.nobs = n_per_condition
    )
