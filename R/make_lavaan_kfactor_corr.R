...
.make_lavaan_kfactor_corr <- function(
  ...
  .one_block <- function(n_i, alpha_i, lv_var_i, x_var_i, factor_i, prefix_i) {
    ...
    if (resid <= 0) {
      stop(
        "Non-positive residual variance for ",
        factor_i,
  ...
  if (K >= 2) {
    ...
    if (is.matrix(corr_latent)) {
      ...
      if (length(r) != 1 || !is.finite(r) || abs(r) >= 1) {
        stop(
          "Scalar corr_latent must be a finite value strictly between -1 and 1."
        )
      }
      if (r <= -1 / (K - 1)) {
        stop(sprintf(
          "For equicorrelation, r must be > -1/(K-1) = %.3f.",
          -1 / (K - 1)
        ))
      }
      R <- matrix(r, K, K)
